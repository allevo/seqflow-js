...
Every SeqFlow component accepts as a second parameter its own context object that exposes some functions to interact with the DOM. In this case we want to render the quote, so we use the `component.sync` method to render the quote in the browser. As you can see, SeqFlow supports JSX syntax.
...
SeqFlow components can replace their own content just by calling the `sync` method again. In this case, the loading message is replaced by the quote component or the error message. Nice, right?
