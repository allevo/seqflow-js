...
function startComponent<T extends { children?: ChildenType[]; key?: string }>(
        ...
        const childContext: SeqflowFunctionContext = {
                ...
                waitEvents: async function* <A>(
                        ...
                        for await (const ev of combineEvents(
                ...
                replaceChild(
                        ...
                        const oldChildIndex = componentChildren.findIndex(
                                (c) => c.key === key
                        );
                        if (oldChildIndex < 0) {
                                this.app.log.error({
                                ...
                                throw new Error(
                                        "replaceChild: wrapper not found"
                                );
                        }
                        ...
                        const [oldChild] = componentChildren.splice(
                                oldChildIndex,
                                1
                        );
                ...
                >(
                        ...
                        const eventTarget =
                                parentContext.app.domainEventTargets[
                                        domainName
                                ];
...
>(
        ...
        seqflowConfig.router
                .getEventTarget()
                .addEventListener("navigation", (ev) => {
                        appContext.log.info({
                                message: "navigate",
                                data: {
        ...
        const mainContext: SeqflowFunctionContext = {
                ...
                waitEvents: async function* <A>(): AsyncGenerator<A> {
                        throw new Error(
                                "waitEvents is not supported in the main context"
                        );
                },
                replaceChild: () => {
                        throw new Error(
                                "replaceChild is not supported in the main context"
                        );
                },
                domEvent: <
                ...
                >(): EventAsyncGenerator<HTMLElementEventMap[K]> => {
                        throw new Error(
                                "domEvent is not supported in the main context"
                        );
                },
                domainEvent<
                        BEE extends typeof DomainsPackage.DomainEvent<unknown>
                >(): EventAsyncGenerator<InstanceType<BEE>> {
                        throw new Error(
                                "domainEvent is not supported in the main context"
                        );
                },
                navigationEvent(): EventAsyncGenerator<NavigationEvent> {
                        throw new Error(
                                "routerEvent is not supported in the main context"
                        );
                },
                createDOMFragment(): DocumentFragment {
                        throw new Error(
                                "createDOMFragment is not supported in the main context"
                        );
                },
                createDOMElement(
                        tagName: string,
...
function applyDefaults(
        ...
        if (seqflowConfiguration.router === undefined) {
                seqflowConfiguration.router = new BrowserRouter(
                        new EventTarget()
                );
        }
...
function createDomains(
        ...
        for (const domainKey of domainKeys as (keyof Domains)[]) {
                ...
                const c = domainFunctions[
                        domainKey as keyof Domains
                ] as Function;
                domains[domainKey] = c(
