...
import { createAbortableEventAsyncGenerator, combineEventAsyncGenerators, createCustomEventAsyncGenerator } from '../../src/events';
...
test('events: combineEventAsyncGenerators with many', async () => {
    ...
    const gens = eventTargets.map(ev => createAbortableEventAsyncGenerator(ev, eventName, {}))
