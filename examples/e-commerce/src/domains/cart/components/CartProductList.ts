...
export async function CartProduct({
	...
	event,
	domains,
}: ComponentParam<{ product: Product; count: number; subTotal: number }>) {
	dom.render(`
	...
	for await (const ev of events) {
		if (button.contains(ev.target as Node)) {
			...
			event.dispatchDomainEvent(
				new ChangeCartEvent({ product: data.product, action: "remove-all" }),
			);
		}
	}
}
...
<div id="cart-product-list">
	<ul class="${classes.cartProducts}" id="cart-products">
		${data.cart.products
			.map(({ product: { id } }) => `<li id="cart-item-${id}"></li>`)
			.join("")}
	</ul>
	<hr />
	...
	for (const { product, count, subTotal } of data.cart.products) {
		dom.child(`cart-item-${product.id}`, CartProduct, {
			data: { product, count, subTotal },
		});
	}
	...
	for await (const ev of events) {
		...
		if (ev instanceof ChangeCartEvent) {
			...
			cartProducts.innerHTML = cart.products
				.map(({ product: { id } }) => `<li id="cart-item-${id}"></li>`)
				.join("");
