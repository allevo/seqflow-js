...
export const CounterChanged = createDomainEventClass<{ delta: number, counter: number }>('counter', 'changed')
export const CounterReset = createDomainEventClass<{ counter: number }>('counter', 'reset')

export class CounterDomain {
  private counter: number

  constructor(private eventTarget: EventTarget, private init: number = 0) {
    this.counter = init
  }
